...
| **Chord Library** | Every chord as a grip hung from a root you can name. Pick a root (buttons or tap the board — that also picks the string and the spot), pick a flavor (the shape appears floating, then snaps when the root lands), or both — then cycle every playable voicing, easiest first. Strum or arpeggiate any grip, ghost your pentatonic skeleton behind it, and read what it's built from in degree language. |
